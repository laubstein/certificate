...
public class ADRBCMS_2_1 implements SignaturePolicy {
	...
	public void validate(byte[] content, byte[] contentSigned) {
		...
		if (signedSigningCertificate != null) {
			...
			throw new SignerException(Messages.getString("erro.atributoSigningCertificateNulo"));
		}
		...
		if (dataSigner != null) {
			if (dataSigner.before(firstDate)) {
				throw new SignerException(Messages.getString("error.unvalidSigningTimeNotValidBefore", "06/03/2011"));
			}
			if (dataSigner.after(lastDate)) {
				throw new SignerException(Messages.getString("error.unvalidSigningTimeNotValidBefore", "21/06/2023"));
			}
		} else {
